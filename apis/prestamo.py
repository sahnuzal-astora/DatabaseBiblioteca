...
async def obtener_prestamo(
    ...
    try:
        ...
        prestamo = prestamo_crud.obtener_prestamos_usuario(skip=skip, limit=limit)
        return prestamo
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error al obtener prestamo: {str(e)}",
        )
...
async def obtener_prestamo(prestamo_id : UUID, db: Session = Depends(get_db)):
    ...
    try:
        prestamo_crud = PrestamoCRUD(db)
        prestamo = prestamo_crud.obtener_prestamos_usuario(prestamo_id)
        if not prestamo:
            raise HTTPException(
